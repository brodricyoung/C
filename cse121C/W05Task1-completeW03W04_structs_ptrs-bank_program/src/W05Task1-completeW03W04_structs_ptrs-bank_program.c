...
struct bank* find_account(bank *head) {
	int num;
	printf("\nEnter the account number to find: ");
	scanf("%d", &num);

	struct bank *found = NULL;
	struct bank *a = head;
	while (a != NULL) {
...
void update_account(bank **head) {
	struct bank *found = find_account(*head);
	if (found != NULL) {
		struct bank *next_account = found->next;

		struct bank *prev_account = *head;
		if (prev_account != found) {
			while (prev_account->next != found) {
				prev_account = prev_account->next;
			}
		}
		free(found);
		...
		struct bank *updated_account = *head;
		while (updated_account->next != updated_account) {
			updated_account = updated_account->next;
		}
		updated_account->next = next_account;
	}
}
